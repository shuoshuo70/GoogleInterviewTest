/**
 * Created by shuoshu on 2018/3/17.
 */
public class LineIntersect {
    class Point {
        int x, y;
        Point (int x, int y) {
            this.x = x;
            this.y = y;
        }
    }

    private int orientation(Point p, Point q, Point r) {
        int val = (q.y - p.y) * (r.x - q.x) - (r.y - q.y) * (q.x - p.x);
        if (val == 0) {
            return 0;
        }

        return val < 0 ? 1 : 2;
    }

    private boolean inSegment(Point p, Point q, Point r) {
        return q.x <= Math.max(p.x, r.x) && q.x >= Math.min(p.x, r.x) && q.y <= Math.max(p.y, r.y) && q.y >= Math.min(p.y, r.y);
    }

    public boolean isIntersect(Point p1, Point q1, Point p2, Point q2) {
        int o1 = orientation(p1, q1, p2);
        int o2 = orientation(p1, q1, q2);
        int o3 = orientation(p2, q2, p1);
        int o4 = orientation(p2, q2, q1);

        if (o1 != o2 && o3 != o4) {
            return true;
        }
        if (o1 == 0 && inSegment(p1, p2, q1)) {
            return true;
        }
        if (o2 == 0 && inSegment(p1, q2, q1)) {
            return true;
        }
        if (o3 == 0 && inSegment(p2, p1, q2)) {
            return true;
        }
        if (o4 == 0 && inSegment(p2, q1, q2)) {
            return true;
        }
        return false;
    }

}
